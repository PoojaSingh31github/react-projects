  ...
  return (
    <>
      <select
        ...
        className="p-2.5 pr-8 text-sm bg-white border border-gray-300 rounded-l"
      >
        {categories?.map((e: string, i: number) => (
          <option key={i} value={e} className="bg-white text-gray-800 dark:bg-gray-700 dark:text-white">
            {e}
          </option>
        ))}
      </select>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
        {articles?.map((article: any, i: number) => (
          <div
            key={i}
            className="bg-white rounded-xl shadow-md overflow-hidden transition transform hover:scale-[1.01] hover:shadow-lg duration-300"
      ...
      <div className="flex justify-center space-x-4 my-4">
        ...
        {Array.from({ length: Math.ceil(totalData / 20)}).map((_, index) => (
          <button
            ...
            className={`px-4 py-2 border ${page === index + 1 ? 'bg-blue-600 text-white' : 'bg-white text-gray-600'} rounded-lg hover:bg-blue-500 hover:text-white`}
          >
            {index + 1}
          </button>
        ))}
